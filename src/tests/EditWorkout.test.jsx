...
describe("EditWorkout", () => {
  ...
  it("hides remove exercise button when only one exercise remains", async () => {
    ...
    const removeExerciseButtons = screen.getAllByRole("button", {
    ...
    expect(
      screen.queryByRole("button", { name: /Remove Exercise/ }),
  ...
  it("shows errors when edit workout has empty or invalid inputs", async () => {
    ...
    expect(
      await screen.findByText(/Weight is required\./i),
    ).toBeInTheDocument();
    expect(
      await screen.findByText(/Duration \(seconds\) is required\./i),
    ).toBeInTheDocument();
  ...
  it("calls supabase.rpc() with the correct data upon form submission", async () => {
    ...
    await waitFor(() => {
      expect(mockRpc).toHaveBeenCalledWith("edit_workout", mockPayload);
    });
  ...
  it("inserts a new exercise (if found) when a new exercise is added", async () => {
    ...
    await waitFor(() => {
      expect(mockRpc).toHaveBeenCalledWith("edit_workout", newExercisePayload);
    });
  ...
  it("removes an exercise and sends updated data to supabase with the correct shape", async () => {
    ...
    await waitFor(() => {
      expect(mockRpc).toHaveBeenCalledWith("edit_workout", updatedPayload);
    });
