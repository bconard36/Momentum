...
const WorkoutLog = ({
   ...
   */
  const monthOptions = [
    ...new Set(savedWorkouts.map((workout) => workout.date.split("-")[1])),
  ].sort();
  ...
  if (workoutError) {
    return (
      <div className="log-fetch-error-container">
        ...
        <button className="secondary-button" onClick={fetchWorkoutLog}>
          Try Again
        </button>
      </div>
    );
      ...
      {savedWorkouts?.length === 0 && (
        <div className="workout-modal-overlay">
          <div
            ...
            role="dialog"
            aria-modal="true"
            aria-label="Workout Log Results"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="empty-log-return">
              <p>No workouts logged yet.</p>
              <Link to="/dashboard" className="return-link empty-return">
                Return to Dashboard
              </Link>
            </div>
          </div>
      ...
      {savedWorkouts?.length > 0 && (
        <>
          {deleteConfirm && (
            ...
            >
              <p className="success-message">Your workout has been deleted.</p>
              <div className="success-return-container">
                <button
                  className="secondary-button success-redirect"
                  type="button"
                ...
                >
                  ...
                  {sortDates.map((workout, index) => (
                    <div key={index}>
                      ...
                      <button
                        type="button"
                        className="secondary-button delete-workout"
                        onClick={() => setPendingDelete(workout.workout_id)}
                      >
                        Delete Workout
                      </button>
                      ...
                      {pendingDelete === workout.workout_id && (
                        <div className="workout-modal-overlay delete-overlay">
                          ...
                          <button
                            type="button"
                            className="secondary-button confirm-delete-workout"
